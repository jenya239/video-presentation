...
- **Улучшенное управление**:
  - Мышь → ракетка точно следует за курсором (raycasting от камеры)
  - Клик мыши → запуск мяча
  - Кастомный crosshair (неоновый круг)
- **Камера**: вид сверху под углом для лучшего обзора игрового поля
- **Физика**:
  - Реалистичные отскоки от стен, ракетки, блоков
  - Спин при ударе сбоку ракетки
  - Постепенное ускорение мяча
- **Система прогрессии**:
  - 5 уникальных уровней с разными паттернами:
    1. **Pyramid** - классическая пирамида
    2. **Checkerboard** - шахматная доска
    3. **Diamond** - ромб с градиентом
    4. **Zigzag** - зигзаг паттерн
    5. **Full Wall** - финальная стена из всех блоков
  - 3 жизни
...
## 8. Tower Defense (`tower-defense.html`)

Простая 3D Tower Defense игра с управлением мышью:

- **Центральная турель**: 
  - Управление огнём мышью (прицел следует за курсором)
  - Снаряды вылетают из дула
  - Эффект отдачи при выстреле
  - Периодический запуск ракет с area damage
- **Автопрогресс**: 
  - Каждые 20 секунд турель улучшается (урон, скорость стрельбы)
  - Враги становятся сильнее с каждой волной
  - Волны запускаются автоматически
- **Враги (октаэдры)**: 
  - Спавнятся со всех сторон
  - Движутся к центральной турели с вращением
  - Отбрасываются назад при попадании (knockback)
  - Усиливаются с каждой волной (HP, скорость, урон, размер)
- **Оптимизация**: 
  - Минимальная геометрия (треугольники)
  - Лимиты на пули (20) и частицы (20)
  - Reuse геометрий и материалов
- **Визуал**: 
  - Система частиц для взрывов
  - HUD с детальной статистикой
  - Crosshair для прицеливания
- **Game Over**: 
  - Когда враги разрушают турель
  - Кнопка "Rebuild Tower" для рестарта
